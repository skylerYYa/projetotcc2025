        ...
        <motion.div className="flex flex-col sm:flex-row justify-between items-center mb-6 pb-4 border-b border-slate-300">
          ...
          <motion.button
            ...
            onClick={() => {
              ...
              setEditingUser(null); // Fecha edição se abrir novo
            }}
                ...
                <tbody>
                  {users.map((user) => (
                    <tr key={user.id} className="border-b border-slate-300 hover:bg-slate-100">
                      <td className="py-3 px-6">{user.nome || "Sem nome"}</td>
                      <td className="py-3 px-6">{user.email}</td>
                      <td className="py-3 px-6">{user.nivelAcesso || "Sem função definida"}</td>
                      <td className="py-3 px-6 text-center">
                        <button
                          onClick={() => handleToggleStatus(user.id, user.statusUsuario)}
